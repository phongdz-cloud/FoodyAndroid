...
import hcmute.edu.vn.foody_10.Adapter.FindOrderAdapter;
...
public class FindOrdersFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        tvEmptyFriendsList = view.findViewById(R.id.tvEmptyOrder);
    ...
    public static void loadOrderdata() {
        if (Common.currentUserModel != null) {
            List<OrderModel> orders = orderQuery.findOrderByUserId(Common.currentUserModel.getId());
            if (orders != null) {
    ...
    public static void clickBtnPlusCount(Context context, OrderModel orderModel) {
        try {
            ...
            if (updateOrder != null) {
                ...
                orderModels.clear();
                orderModels.addAll(orderQuery.findOrderByUserId(Common.currentUserModel.getId()));
                findOrderAdapter.notifyDataSetChanged();
            }
    ...
    public static void insertOrUpdateOrder(Context context, FoodModel foodModel) {
        try {
            orderModels.clear();
            final OrderModel orderModel = orderQuery.findByProductIdAndUserId(foodModel.getId(), Common.currentUserModel.getId());
            if (orderModel == null) {
                OrderModel orderAdd = new OrderModel(null, foodModel.getPhotoFood(),
                        1, foodModel.getFoodName(), foodModel.getFoodDescription(),
                        foodModel.getPrice(), foodModel.getId(), Common.currentUserModel.getId());
